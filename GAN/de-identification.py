...
dir_to_base = 'GAN/'                                                #*may require editing
...
G = Generator()
if constraint==0:
    G.load_state_dict(torch.load(dir_to_base + '200000-G.ckpt', map_location=map_location))
    G.main[0] = nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=3, bias=False) #Disabled Editing
    if datatype=='Depth':                                                          #Enforce 1 channel output
        G.main[21] = nn.Conv2d(64, 1, kernel_size=7, stride=1, padding=3, bias=False)
else:
    G.main[0] = nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=3, bias=False) #Disabled Editing
    if datatype=='Depth':                                                          #Enforce 1 channel output
        G.main[21] = nn.Conv2d(64, 1, kernel_size=7, stride=1, padding=3, bias=False)
    G.load_state_dict(torch.load(dir_to_base + 'constraint-'+datatype+'/'+ 'pretrain/'+'model-G.pth'))
G.to(device)

## Discriminator:
from stargan import Discriminator
D = Discriminator()
if not constraint==0:
    D.load_state_dict(torch.load(dir_to_base + 'constraint-'+datatype+'/'+ 'pretrain/'+'model-D.pth'))
else:
    D.load_state_dict(torch.load(dir_to_base + '200000-D.ckpt', map_location=map_location))
D.to(device)
...
g_optimizer = torch.optim.Adam(G.parameters(), lr=0.0001) #0.0001
d_optimizer = torch.optim.Adam(D.parameters(), lr=0.0001) #0.0001
...
for epoch in range(epoch_start, num_epochs):
    for i, (X, y1, y2, y3, y4, X_path) in enumerate(train_dataloader):
        ...
        lambda_adv = 1
        lambda_gp = 10
        d_loss = lambda_adv*(d_loss_real + d_loss_fake) + lambda_gp * d_loss_gp
        ...
        if (i+1) % 5 == 0:
            ...
            if constraint == 0:
                ...
                g_loss_constraint = loss_fn(model(x_fake), y1)
            elif constraint == 2:
                g_loss_constraint = loss_fn(model(x_fake), y2)
            elif constraint == 3:
                g_loss_constraint = loss_fn(model(x_fake), y3)
            elif constraint == 4:
                g_loss_constraint = loss_fn(model(x_fake), y4)
            elif constraint == 5:
                g_loss_constraint = loss_fn(model2(x_fake),y2)+loss_fn(model3(x_fake),y3)+loss_fn(model4(x_fake),y4)
            elif constraint == 6:
                loss_constraint_att = loss_fn(model2(x_fake), y2)+loss_fn(model3(x_fake),y3)+loss_fn(model4(x_fake),y4)
                loss_constraint_id = loss_fn(model1(x_fake), y1)
                g_loss_constraint = -para/10*loss_constraint_id+loss_constraint_att

            g_loss_rec = torch.mean(torch.abs(x_real - x_fake))

            # Backward and optimize.
            lambda_adv = 1
            lambda_rec = 10
            lambda_constraint = 10
            g_loss = lambda_adv*g_loss_fake + lambda_rec * g_loss_rec + lambda_constraint * g_loss_constraint
            ...
            if constraint == 0:
                loss['G/loss_constraint'] = g_loss_constraint
            else:
                loss['G/loss_constraint'] = g_loss_constraint.item()
        ...
        if (i+1) % 20 == 0:
            log = "Epoch [{}/{}], Iteration [{}/{}]".format(epoch+1, num_epochs, i+1, len(train_dataloader))
            for tag, value in loss.items():
                log += ", {}: {:.4f}".format(tag, value)
            print(log)
    

torch.save(G.state_dict(), os.path.join(out_path, 'model-G.pth'))
torch.save(D.state_dict(), os.path.join(out_path, 'model-D.pth'))
